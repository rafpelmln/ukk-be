<x-app-layout>
    <x-slot name="header">
        <div class="flex w-full flex-col gap-3">
            <h1 class="text-3xl font-semibold text-slate-900 dark:text-white">Edit Banner Beranda</h1>
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <form action="{{ route('home-banners.update', $banner) }}" method="POST" enctype="multipart/form-data" data-banner-form="edit">
                @include('home-banners._form', [
                    'method' => 'PUT',
                    'banner' => $banner,
                    'submitLabel' => 'Perbarui Banner',
                ])
            </form>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const form = document.querySelector('form[data-banner-form="edit"]');
