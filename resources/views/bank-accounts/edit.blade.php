<x-app-layout>
    <x-slot name="header">
        <div class="flex w-full flex-col gap-3">
            <h1 class="text-3xl font-semibold text-slate-900 dark:text-white">Edit Rekening Bank</h1>
    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <form action="{{ route('bank-accounts.update', $bankAccount) }}" method="POST" enctype="multipart/form-data" data-bank-account-form="edit">
                @csrf
                @method('PUT')
                <div class="grid gap-4">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <label class="block">
                            ...
                            <input
                                ...
                                name="nama_bank"
                                id="nama_bank"
                                value="{{ old('nama_bank', $bankAccount->nama_bank) }}"
                        ...
                        <label class="block">
                            ...
                            <input
                                type="text"
                                name="nama"
                                value="{{ old('nama', $bankAccount->nama) }}"
                                class="mt-1 w-full rounded-md border-slate-200 px-3 py-2"
                    ...
                    <label class="block sm:w-1/2">
                        ...
                        <input
                            type="text"
                            name="no_rek"
                            value="{{ old('no_rek', $bankAccount->no_rek) }}"
                            class="mt-1 w-full rounded-md border-slate-200 px-3 py-2"
                    ...
                    <div class="space-y-3">
                        <label class="block">
                            ...
                            <input
                                ...
                                class="mt-1"
                            >
                            <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Ukuran maksimum 2 MB, akan dikompresi otomatis ke sekitar 250 KB. Kosongkan jika tidak ingin mengubah foto.</p>
                        ...
                        @if ($bankAccount->photo_url)
                            <div class="flex items-center gap-4">
                                <span class="text-sm text-slate-700 dark:text-slate-300">Logo saat ini:</span>
                                <img src="{{ $bankAccount->photo_url }}" alt="{{ $bankAccount->nama_bank }}" class="h-16 w-24 rounded-lg border border-slate-200 object-contain p-2">
                            </div>
                        @endif
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const form = document.querySelector('form[data-bank-account-form="edit"]');
